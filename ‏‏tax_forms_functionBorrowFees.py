...
# Borrow fees
# ---------------------------------------------------------------------------
def extract_borrow_fees_by_ticker(csv_file, date_slash_format='normal'):
    """
    Parse Borrow Fee Details rows and return a dict:
      { (ticker, abs_quantity): total_fee }
    """
    ...
    with open(csv_file, 'r') as f:
        for row in csv.reader(f):
            if len(row) < 10:
                continue
            if row[0].strip() != 'Borrow Fee Details' or row[1].strip() != 'Data':
                continue
            ...
            ticker = row[4].strip()
            try:
                qty    = abs(int(float(row[5])))
                amount = float(row[9])
            except (ValueError, IndexError):
                continue
            if amount == 0:
                continue
            fees[(ticker, qty)] += amount
    return fees


def get_borrow_fee_for_lot(borrow_fees, ticker, quantity):
    """
    Look up borrow fee by (ticker, abs_quantity).
    Returns the total fee (negative float) or 0.0 if not found.
    """
    return borrow_fees.get((ticker, abs(int(quantity))), 0.0)
...
def build_closed_lot_dict(closed_lot_trade, closing_trade, allocated_close_fee, rate_provider, opening_resolution=None, borrow_fee=0.0):
    ...
    if is_long:
        ...
        consideration_value = abs(opening_basis) + abs(borrow_fee)
...
def extract_trades_data_from_csv(file_dir, csv_file_name, verbosity=0, date_slash_format='normal'):
    ...
    borrow_fees   = extract_borrow_fees_by_ticker(csv_file, date_slash_format=date_slash_format)
    ...
    for closing_trade, lots in closing_trade_pairs:
        ...
        for lot in lots:
            ...
            ticker_idx[ticker] += 1
            borrow_fee = get_borrow_fee_for_lot(borrow_fees, ticker, lot['quantity']) if lot['quantity'] < 0 else 0.0
            d = build_closed_lot_dict(lot, closing_trade, fee, rate_provider, resolution, borrow_fee=borrow_fee)
                ...
                if section == 'Commission Details':
                    ...
                    if entry:
                        if 'Borrow Fee' in description:
                            pass  # capitalized into short lot cost via Borrow Fee Details — skip here
...
def _write_unmatched_wht_table(sheet, unmatched_wht, start_row, col_start=2, col_end=10):
    ...
    _style_header_row(sheet, start_row, col_start, col_end, 'מס שנוכה במקור והוחזר', PRIOR_FILL)
