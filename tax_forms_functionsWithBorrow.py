...
def build_closed_lot_dict(closed_lot_trade, closing_trade, allocated_close_fee, col_names, rate_provider, opening_resolution=None, borrow_fee=0.0):
...
def extract_trades_data_from_csv(file_dir, csv_file_name, verbosity=0, date_slash_format='normal'):
    ...
    borrow_fees_by_ticker = extract_borrow_fees_by_ticker(csv_file, date_slash_format=date_slash_format)
    ...
    if col_names:
        ...
        for closing_trade, closed_lot_trades in closing_trade_pairs:
            for lot, closing_trade_inner, allocated_close_fee in [
                ...
                borrow_fee = get_borrow_fee_for_lot(borrow_fees_by_ticker, lot['ticker'], lot['quantity']) if lot['quantity'] < 0 else 0.0
                closed_lot_dict = build_closed_lot_dict(
